...
class Asset(object):
    def __init__(self, assetId):
        self.assetId='' if assetId == 'LUNES' else assetId
    ...
    def status(self):
        if self.assetId!='LUNES':
            try:
                req = lunespy.wrapper('/transactions/info/%s' % self.assetId)
                if req['type'] == 3:
...
class AssetPair(object):
    def __init__(self, asset1, asset2):
        self.asset1 = asset1
        self.asset2 = asset2
        self.a1 = 'LUNES' if self.asset1.assetId == '' else self.asset1.assetId
        self.a2 = 'LUNES' if self.asset2.assetId == '' else self.asset2.assetId
    ...
    def orderbook(self):
        req = lunespy.wrapper('/matcher/orderbook/%s/%s' % (self.a1, self.a2), host=lunespy.MATCHER)
        return req

    def ticker(self):
        return lunespy.wrapper('/api/ticker/%s/%s' % (self.a1, self.a2), host=lunespy.DATAFEED)
    ...
    def _getMarketData(self, method, params):
        return lunespy.wrapper('%s/%s/%s/%s' % (method, self.a1, self.a2, params), host=lunespy.DATAFEED)

    def trades(self, *args):
        if len(args)==1:
            limit = args[0]
            if limit > 0 and limit <= lunespy.MAX_WDF_REQUEST:
    ...
    def candles(self, *args):
        if len(args)==2:
            timeframe = args[0]
            limit = args[1]
            if timeframe not in lunespy.VALID_TIMEFRAMES:
                return logging.error('Invalid timeframe')
            elif limit > 0 and limit <= lunespy.MAX_WDF_REQUEST:
